...
// Same monochrome Bootstrap Icons (MIT licensed) used in HowItWorks.jsx,
...
// Small inline checkmark, reused for both the bullet list and the
// "resolved" row state in the lead queue panel.
...
// Three rows, three outcomes: one about to slip through, one caught as a
// duplicate before anyone double-replies, one already handled well. Same
// states DashboardPage.jsx tracks for real (status, possible_duplicate).
const leads = [
...
// id="product" makes the Header's "Product" nav button actually scroll
// somewhere — it previously pointed at nothing, since UnifiedInbox.jsx
// (the only other element that ever carried this id) isn't rendered on
// the page.
        ...
        >
            <style>{`
                ...
                @media (max-width: 900px) {
                    .ek-kc-grid { grid-template-columns: 1fr !important; gap: 40px !important; }
                    .ek-kc-visual { aspect-ratio: auto !important; padding: 28px 24px !important; }
                }
            ...
            >
                {/* Left: copy + bullets, no card — matches Arlo's plain
                    copy column treatment for its two-column feature
                    sections (the card styling lives on the visual only). */}
                <div>
                    <h2
                        style={{
                ...
                {/* Right: lead queue panel — sized like Arlo's real
                    .hs-panel (aspect-ratio 539/452, width:100% of the
                    column, 26px radius) instead of shrink-wrapping to its
                    content, so it carries the same visual weight as the
                    reference and the other full-size panels on this site
                    (HowItWorks, OnePlace), rather than reading as a small
                    inset box next to the copy. */}
                <div
                ...
                >
                    ...
                    {/* Rows fill the remaining height and centre within it —
                        same flex-column + justify-content:center pattern as
                        Arlo's own .hs-panel--prompts, just with cards instead
